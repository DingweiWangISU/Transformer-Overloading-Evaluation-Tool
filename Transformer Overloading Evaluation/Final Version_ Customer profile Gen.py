...
AMI_Hourly= pd.read_excel("Adjusted_AMI_Data.xlsx")
...
Pen_Level = 35  # Adjust this value as needed
...
selected_customers = random.sample(list(AMI_Hourly.columns[4:]), Pen_Level)
...
for customer in selected_customers:
    ...
    AMI_Hourly_updated = replace_customer_profile(AMI_Hourly, Customer_Total_Usage, customer_name)
    
    # Display the updated AMI_Hourly to verify the changes
    print(AMI_Hourly_updated.head())
    


# Define the output file path with the penetration level in the file name
output_file_path = f"output\Final Aggregated Data_PenLevel_{Pen_Level}.xlsx"

# Use pandas to export the DataFrame to an Excel file
AMI_Hourly_updated.to_excel(output_file_path, index=False)
